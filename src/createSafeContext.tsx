 ...
 *
 * @template T The type of value the context should hold
 */
...
const safeConsumer =
  <T,>(name: string, context: React.Context<T | undefined>) =>
  (props: React.ConsumerProps<T>) => (
    <context.Consumer>{(value) => props.children(validateValue(name, value))}</context.Consumer>
  );

const safeProvider =
  <T,>(context: React.Context<T | undefined>) =>
  (props: React.ProviderProps<T>) => <context.Provider value={props.value}>{props.children}</context.Provider>;
